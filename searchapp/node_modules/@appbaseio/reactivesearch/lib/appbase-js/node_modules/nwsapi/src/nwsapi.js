...
(function Export(global, factory) {
  ...
  isHTML = function isHTML(node) {
    ...
    return doc.nodeType == 9 && doc.contentType.indexOf('/html') > 0 && doc.createElement('DiV').nodeName == 'DIV';
  },
  ...
  setIdentifierSyntax = function setIdentifierSyntax() {
    ...
    '(?:\\*|\\*\\|\\*)|' + '(?!:not)' + '(?:' + WSP + '?[.:#]?' + '(?:' + identifier + WSP + '?)+|' + '(?:\\x28[^()]*(?:\\x29|$))' + ')+|' + '(?:' + WSP + '?' + attributes + WSP + '?)+|' + ')$');
    ...
    reValidator = RegExp(Config.SIMPLENOT ? standardValidator : extendedValidator, 'g');
  },
    ...
    while (selector) {
      ...
      switch (symbol) {
        ...
        case '#':
          ...
          source = 'if(' + N + '(e.getAttribute("id")=="' + convertEscapes(match[1]) + '"' + ')){' + source + '}';
          break;
        // class name resolver
        case '.':
        ...
        case symbol.match(/[a-zA-Z]/) ? symbol : undefined:
          ...
          source = 'if(' + N + '(e.nodeName.toLowerCase()=="' + match[1].toLowerCase() + '"' + ')){' + source + '}';
          break;
        // namespace resolver
        case '|':
        ...
        case '[':
          ...
          source = 'if(' + N + '(' + (!match[2] ? 'e.hasAttribute("' + name + '")' : !match[4] && ATTR_STD_OPS[match[2]] && match[2] != '~=' ? 'e.getAttribute("' + name + '")==""' : '(/' + test.p1 + match[4] + test.p2 + '/' + type + ').test(e.getAttribute("' + name + '"))==' + test.p3) + ')){' + source + '}';
          break;
  ...
  parseGroup = function parseGroup(selector) {
    var i,
        l,
        groups = selector.replace(/,\s?,/g, ',').replace(/\\,/g, '\uFFFD').split(REX.SplitGroup);
    for (i = 0, l = groups.length; l > i; ++i) {
  ...
  match = function _matches(selector, element, callback) {
    ...
    selector = selector.replace(/\x00|\\$/g, '\uFFFD').replace(REX.CombineWSP, '\x20').replace(REX.TabCharWSP, '\t').replace(REX.CommaGroup, ',').replace(REX.TrimSpaces, '');
  ...
  select = function _querySelectorAll(selector, context, callback) {
    ...
    resolver = collect(groups.length < 2 ? selector : groups, context, callback, HTML_DOCUMENT && context.nodeType != 11 ? domapi : compat);
