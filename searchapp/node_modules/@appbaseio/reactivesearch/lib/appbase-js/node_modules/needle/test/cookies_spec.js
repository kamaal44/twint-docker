...
describe('cookies', function () {
  ...
  before(function () {
    setCookieHeader = [WEIRD_COOKIE_NAME + '=' + encode(WEIRD_COOKIE_VALUE) + ';', BASE64_COOKIE_NAME + '=' + encode(BASE64_COOKIE_VALUE) + ';', FORBIDDEN_COOKIE_NAME + '=' + encode(FORBIDDEN_COOKIE_VALUE) + ';', NUMBER_COOKIE_NAME + '=' + encode(NUMBER_COOKIE_VALUE) + ';'];
  });
  ...
  describe('with default options', function () {
    it('no cookie header is set on request', function (done) {
      needle.get(TEST_HOST + ':' + ALL_COOKIES_TEST_PORT, function (err, response) {
  ...
  describe('if response does not contain cookies', function () {
    ...
    it('response.cookies is undefined', function (done) {
      needle.get(TEST_HOST + ':' + NO_COOKIES_TEST_PORT, function (error, response) {
  ...
  describe('if response contains cookies', function () {
    ...
    it('parses them as a object', function (done) {
      needle.get(TEST_HOST + ':' + ALL_COOKIES_TEST_PORT, function (error, response) {
        response.cookies.should.be.an.instanceOf(Object).and.have.property(WEIRD_COOKIE_NAME);
    ...
    describe('when a cookie value is invalid', function () {
      ...
      before(function () {
        setCookieHeader = ['geo_city=%D1%E0%ED%EA%F2-%CF%E5%F2%E5%F0%E1%F3%F0%E3'];
      });
    ...
    describe('and response is a redirect', function () {
      ...
      var responseCookies = [[// first req
      WEIRD_COOKIE_NAME + '=' + encode(WEIRD_COOKIE_VALUE) + ';', BASE64_COOKIE_NAME + '=' + encode(BASE64_COOKIE_VALUE) + ';', 'FOO=123;'], [// second req
      FORBIDDEN_COOKIE_NAME + '=' + encode(FORBIDDEN_COOKIE_VALUE) + ';', NUMBER_COOKIE_NAME + '=' + encode(NUMBER_COOKIE_VALUE) + ';'], [// third red
      'FOO=BAR;']];
    ...
    describe('with parse_cookies = false', function () {
      it('does not parse them', function (done) {
        needle.get(TEST_HOST + ':' + ALL_COOKIES_TEST_PORT, { parse_cookies: false }, function (error, response) {
          assert(!response.cookies);
          done();
        });
      });
  ...
  describe('if request contains cookie header', function () {
    ...
    it('must be a valid cookie string', function (done) {
      ...
      var full_header = [WEIRD_COOKIE_NAME + '=' + WEIRD_COOKIE_VALUE, BASE64_COOKIE_NAME + '=' + BASE64_COOKIE_VALUE, FORBIDDEN_COOKIE_NAME + '=' + encode(FORBIDDEN_COOKIE_VALUE), NUMBER_COOKIE_NAME + '=' + NUMBER_COOKIE_VALUE].join('; ');
    ...
    it('must encode forbidden characters', function (done) {
      ...
      needle.get(TEST_HOST + ':' + ALL_COOKIES_TEST_PORT, opts, function (error, response) {
        ...
        cookieObj[FORBIDDEN_COOKIE_NAME].should.not.be.eql(FORBIDDEN_COOKIE_VALUE);
        cookieObj[FORBIDDEN_COOKIE_NAME].should.be.exactly(encode(FORBIDDEN_COOKIE_VALUE));
        cookieObj[FORBIDDEN_COOKIE_NAME].should.be.exactly(encodeURIComponent(FORBIDDEN_COOKIE_VALUE));
        done();
      });
    });
  });
