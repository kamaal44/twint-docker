...
module.exports.args = function (leveldown, test, testCommon) {
  test('test database open no-arg throws', function (t) {
    ...
    t.throws(db.open.bind(db), { name: 'Error', message: 'open() requires a callback argument' }, 'no-arg open() throws');
    t.end();
  });

  test('test callback-less, 1-arg, open() throws', function (t) {
    var db = leveldown(testCommon.location());
    t.throws(db.open.bind(db, {}), { name: 'Error', message: 'open() requires a callback argument' }, 'callback-less, 1-arg open() throws');
    t.end();
  });
};
