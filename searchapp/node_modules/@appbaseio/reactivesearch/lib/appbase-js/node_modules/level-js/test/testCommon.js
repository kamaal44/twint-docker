...
var dbidx = 0,
  ...
  return '_leveldown_test_db_' + dbidx++;
    ...
    lastLocation = function lastLocation() {
  return '_leveldown_test_db_' + dbidx;
},
  ...
  setUp(t); // same cleanup!
    ...
    collectEntries = function collectEntries(iterator, callback) {
  var data = [],
      next = function next() {
  ...
  next();
    ...
    makeExistingDbTest = function makeExistingDbTest(name, test, leveldown, testFn) {
  test(name, function (t) {
    cleanup(function () {
      var loc = location(),
          db = leveldown(loc),
          done = function done(close) {
      ...
      db.open(function (err) {
        ...
        db.batch([{ type: 'put', key: 'one', value: '1' }, { type: 'put', key: 'two', value: '2' }, { type: 'put', key: 'three', value: '3' }], function (err) {
