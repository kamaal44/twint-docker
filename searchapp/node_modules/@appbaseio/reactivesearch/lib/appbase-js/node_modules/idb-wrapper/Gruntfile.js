...
module.exports = function (grunt) {
  ...
  var additionalCSS = ['.navbar { display: none; }', '#toc { top: 0; }'];
  ...
  grunt.initConfig({
    ...
    karma: {
      postbuild: {
        ...
        files: {
          src: ['idbstore.min.js', 'test/**/*spec.js']
        }
      },
      dev: {
        configFile: 'karma.conf.js'
  ...
  var indexTemplate = '<!DOCTYPE html>' + '<html>' + '<title>Available IDBWrapper Documentation</title>' + '<p>Available IDBWrapper documentation:</p>' + '<ul>{{list}}</ul>' + '</html>';
  ...
  grunt.registerTask('docs', ['jsdoc:dist', 'modifyDocs', 'copyLatestDocs', 'writeDocIndex']);

  grunt.registerTask('build', ['jshint', 'karma:dev', 'closurecompiler', 'karma:postbuild']);
};