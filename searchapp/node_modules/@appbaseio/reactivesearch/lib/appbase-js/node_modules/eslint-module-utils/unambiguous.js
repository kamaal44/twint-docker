...
var pattern = /(^|;)\s*(export|import)((\s+\w)|(\s*[{*]))/m;
/**
...
var unambiguousNodeType = /^(Exp|Imp)ort.*Declaration$/;
...
exports.isModule = function isUnambiguousModule(ast) {
  return ast.body.some(function (node) {
    return unambiguousNodeType.test(node.type);
  });
};