...
buster.testCase('Optional LevelDOWN', {
  ...
  'test getLevelDOWN()': function testGetLevelDOWN() {
    var util = require('../lib/util');
    assert.same(util.getLevelDOWN(), require('leveldown'), 'correct leveldown provided');
  },

  'test wrong version': function testWrongVersion() {
    var levelup = require('..');
  ...
  'test no leveldown/package': function testNoLeveldownPackage() {
    var levelup = require('..');
    ...
    assert.exception(levelup.bind(null, '/foo/bar'), function (err) {
      if (err.name != 'LevelUPError') return false;
      if ('Could not locate LevelDOWN, try `npm install leveldown`' != err.message) return false;
      return true;
    });
  },

  'test no leveldown': function testNoLeveldown() {
    var levelup = require('..');
    // simulate an exception from a require() that doesn't resolved a package
