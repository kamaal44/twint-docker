...
buster.testCase('ReadStream', {
  ...
  , 'readStream and then put in nextTick': function readStreamAndThenPutInNextTick(done) {
    this.openTestDatabase(function (db) {
      makeTest(db, process.nextTick, done);
    }.bind(this));
  },
  'readStream and then put in nextTick, defered open': function readStreamAndThenPutInNextTickDeferedOpen(done) {
  ...
  'readStream and then put, defered open': function readStreamAndThenPutDeferedOpen(done) {
    var location = common.nextLocation(),
        db = levelup(location);
  ...
  'readStream and then put': function readStreamAndThenPut(done) {
    this.openTestDatabase(function (db) {
      makeTest(db, function (f) {
        f();
      }, done);
