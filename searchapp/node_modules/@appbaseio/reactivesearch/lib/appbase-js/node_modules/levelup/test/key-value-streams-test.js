...
buster.testCase('Key and Value Streams', {
  ...
  'test .keyStream()': function testKeyStream(done) {
  ...
  'test .readStream({keys:true,values:false})': function testReadStreamKeysTrueValuesFalse(done) {
    this.openTestDatabase(function (db) {
      // execute
      db.batch(this.sourceData.slice(), function (err) {
        refute(err);
  ...
  'test .valueStream()': function testValueStream(done) {
    this.openTestDatabase(function (db) {
      // execute
      db.batch(this.sourceData.slice(), function (err) {
        refute(err);
  ...
  'test .readStream({keys:false,values:true})': function testReadStreamKeysFalseValuesTrue(done) {
    this.openTestDatabase(function (db) {
      // execute
      db.batch(this.sourceData.slice(), function (err) {
        refute(err);
