...
buster.testCase('batch()', {
  ...
  'batch() with multiple puts': function batchWithMultiplePuts(done) {
    this.openTestDatabase(function (db) {
      db.batch([{ type: 'put', key: 'foo', value: 'afoovalue' }, { type: 'put', key: 'bar', value: 'abarvalue' }, { type: 'put', key: 'baz', value: 'abazvalue' }], function (err) {
        refute(err);
  ...
  'batch() with multiple puts and deletes': function batchWithMultiplePutsAndDeletes(done) {
    this.openTestDatabase(function (db) {
      async.series([function (callback) {
        db.batch([{ type: 'put', key: '1', value: 'one' }, { type: 'put', key: '2', value: 'two' }, { type: 'put', key: '3', value: 'three' }], callback);
      }, function (callback) {
        db.batch([{ type: 'put', key: 'foo', value: 'afoovalue' }, { type: 'del', key: '1' }, { type: 'put', key: 'bar', value: 'abarvalue' }, { type: 'del', key: 'foo' }, { type: 'put', key: 'baz', value: 'abazvalue' }], callback);
      }, function (callback) {
        // these should exist
  ...
  'batch() with chained interface': function batchWithChainedInterface(done) {
    this.openTestDatabase(function (db) {
  ...
  'batch() with can manipulate data from put()': function batchWithCanManipulateDataFromPut(done) {
  ...
  'batch() data can be read with get() and del()': function batchDataCanBeReadWithGetAndDel(done) {
    this.openTestDatabase(function (db) {
      async.series([function (callback) {
        db.batch([{ type: 'put', key: '1', value: 'one' }, { type: 'put', key: '2', value: 'two' }, { type: 'put', key: '3', value: 'three' }], callback);
  ...
  'chained batch() arguments': {
    ...
    'test batch#put() with missing `value`': function testBatchPutWithMissingValue() {
      // value = undefined
      assert.exception(this.batch.put.bind(this.batch, 'foo1'), function (err) {
        ...
        if ('value cannot be `null` or `undefined`' != err.message) return false;
        return true;
      });
    ...
    'test batch#put() with missing `key`': function testBatchPutWithMissingKey() {
      // key = undefined
      assert.exception(this.batch.put.bind(this.batch, undefined, 'foo1'), function (err) {
        if (err.name != 'WriteError') return false;
        if ('key cannot be `null` or `undefined`' != err.message) return false;
        return true;
      });
    ...
    'test batch#put() with missing `key` and `value`': function testBatchPutWithMissingKeyAndValue() {
      // undefined
    ...
    'test batch#del() with missing `key`': function testBatchDelWithMissingKey() {
      // key = undefined
    ...
    'test batch#write() with no callback': function testBatchWriteWithNoCallback() {
      ...
      'setUp': function setUp(done) {
        ...
        this.verify = function (cb) {
          assert.exception(cb, function (err) {
            if (err.name != 'WriteError') return false;
            if ('write() already called on this batch' != err.message) return false;
            return true;
          });
        };
      },

      'test put()': function testPut() {
        this.verify(function () {
          this.batch.put('whoa', 'dude');
        }.bind(this));
      },

      'test del()': function testDel() {
        this.verify(function () {
          this.batch.del('foo');
        }.bind(this));
      },
