...
var _templateObject = _taggedTemplateLiteral(['\n          Scripts must resolve to strings.\n          There is no script that can be resolved from "', '"\n        '], ['\n          Scripts must resolve to strings.\n          There is no script that can be resolved from "', '"\n        ']),
    _templateObject2 = _taggedTemplateLiteral(['\n    ', '\n     `', '`\n     ', '\n  '], ['\n    ', '\n     \\`', '\\`\n     ', '\n  ']),
    _templateObject3 = _taggedTemplateLiteral(['\n            The script called "', '"\n            which runs "', '" emitted an error\n          '], ['\n            The script called "', '"\n            which runs "', '" emitted an error\n          ']),
    _templateObject4 = _taggedTemplateLiteral(['\n              The script called "', '"\n              which runs "', '" failed with exit code ', '\n            '], ['\n              The script called "', '"\n              which runs "', '" failed with exit code ', '\n            ']);
...
function runPackageScripts(_ref) {
  ...
  return scriptNames.reduce(function (res, input) {
    return res.then(function () {
      return runPackageScript({ scriptConfig: scriptConfig, options: options, input: input });
    });
  }, Promise.resolve());
}
...
function runPackageScript(_ref2) {
  ...
  if (!(0, _lodash.isString)(script)) {
    return Promise.reject({
      message: _chalk2.default.red(_commonTags.oneLine(_templateObject, scriptPrefix)),
      ref: 'missing-script'
    });
  }
  ...
  log.info(_commonTags.oneLine(_templateObject2, _chalk2.default.gray('nps is executing'), _chalk2.default.bold(scriptName), showScript ? ': ' + _chalk2.default.green(command) : ''));
  var child = void 0;
  return new Promise(function (resolve, reject) {
    ...
    child.on('error', function (error) {
      reject({
        message: _chalk2.default.red(_commonTags.oneLine(_templateObject3, scriptPrefix, command)),
    ...
    child.on('close', function (code) {
      if (code === NON_ERROR) {
        ...
        reject({
          message: _chalk2.default.red(_commonTags.oneLine(_templateObject4, scriptPrefix, command, code)),
