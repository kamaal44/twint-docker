...
var FSO_ForReading = 1,
    FSO_ForWriting = 2,
    PS_MSG = 'Microsoft Windows PowerShell is required.' + ' https://technet.microsoft.com/en-us/library/hh847837.aspx',
    input = '',
    fso,
    tty,
  ...
  while (typeof (arg = args.shift()) === 'string') {
    ...
    arg = arg.toLowerCase();
    if (confLc[arg]) {
      options[confLc[arg].key] = confLc[arg].type === 'boolean' ? true : confLc[arg].type === 'string' ? args.shift() : null;
    }
  }
  for (key in conf) {
    if (conf.hasOwnProperty(key) && conf[key] === 'string') {
      if (typeof options[key] !== 'string') {
        ...
        options[key] = decodeArg(options[key]);
      }
    }
  }
  return options;
...
function readByPW() {
  ...
  try {
    ...
    .replace(/[\u4000-\u40FF]/g, function (chr) {
      ...
      return charCode >= 0x4020 && charCode <= 0x407F ? String.fromCharCode(charCode - 0x4000) : '';
    });
  } catch (e) {
