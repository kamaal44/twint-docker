...
function getResolveId(extensions) {
	...
	function resolveId(importee, importer) {
		...
		return resolveUsingOtherResolvers(importee, importer).then(function (resolved) {
			if (resolved) {
				return isProxyModule ? PROXY_PREFIX + resolved : resolved;
			}
	...
	resolveId.setRollupOptions = function (options) {
		var resolvers = (options.plugins || []).map(function (plugin) {
		...
		var isExternal = function isExternal(id) {
			return options.external ? Array.isArray(options.external) ? options.external.includes(id) : options.external(id) : false;
		};
...
function transformCommonjs(parse, code, id, isEntry, ignoreGlobal, ignoreRequire, customNamedExports, sourceMap, allowDynamicRequire, astCache) {
	...
	var importBlock = (includeHelpers ? ["import * as " + HELPERS_NAME + " from '" + HELPERS_ID + "';"] : []).concat(sources.map(function (source) {
	...
	function addExport(x) {
		...
		var declaration = deconflicted === name ? "export var " + x + " = " + moduleName + "." + x + ";" : "var " + deconflicted + " = " + moduleName + "." + x + ";\nexport { " + deconflicted + " as " + x + " };";
		...
		ast.body.forEach(function (node) {
			if (node.type === 'ExpressionStatement' && node.expression.type === 'AssignmentExpression') {
				...
				if (flattened.keypath === 'module.exports') {
					...
					var declaration = name === deconflicted ? "export { " + name + " };" : "export { " + deconflicted + " as " + name + " };";
	...
	Object.keys(namedExports).filter(function (key) {
		return !blacklist[key];
	}).forEach(addExport);

	var defaultExport = /__esModule/.test(code) ? "export default " + HELPERS_NAME + ".unwrapExports(" + moduleName + ");" : "export default " + moduleName + ";";

	var named = namedExportDeclarations.filter(function (x) {
...
function commonjs(options) {
	...
	var ignoreRequire = typeof options.ignore === 'function' ? options.ignore : Array.isArray(options.ignore) ? function (id) {
		return options.ignore.includes(id);
	} : function () {
		return false;
	};
	...
	return {
		...
		options: function options(options$1) {
			...
			var entryModules = Array.isArray(input) ? input : (typeof input === 'undefined' ? 'undefined' : _typeof(input)) === 'object' && input !== null ? Object.values(input) : [input];
			entryModuleIdsPromise = Promise.all(entryModules.map(function (entry) {
			...
			if (id.startsWith(PROXY_PREFIX)) {
				...
				return getIsCjsPromise(actualId$1).then(function (isCjs) {
					if (isCjs) {
						...
						return "import * as " + name$1 + " from " + JSON.stringify(actualId$1) + "; import {getCjsExportFromNamespace} from \"" + HELPERS_ID + "\"; export default getCjsExportFromNamespace(" + name$1 + ")";
					}
				});
			}
		},
		...
		transform: function transform(code, id) {
			...
			setIsCjsPromise(id, transformPromise.then(Boolean, function () {
				return false;
			}));
