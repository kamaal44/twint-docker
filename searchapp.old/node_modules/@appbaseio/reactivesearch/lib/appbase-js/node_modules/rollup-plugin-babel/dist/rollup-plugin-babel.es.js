...
function importHelperPlugin(ref) {
  ...
  */
	function replaceWith(path$$1, replacement) {
		if (path$$1.parentPath.isExportDefaultDeclaration() && t.isExpression(replacement)) {
...
function createPreflightCheck() {
	...
	return function (options, dir) {
		if (!preflightCheckResults[dir]) {
			...
			if (!~check.indexOf('export default') && !~check.indexOf('export { Foo as default }')) {
			...
			if (~check.indexOf('import _classCallCheck from')) {
				helpers = RUNTIME;
			} else if (~check.indexOf('function _classCallCheck')) {
				helpers = INLINE;
...
function babel(options) {
	...
	if (options.externalHelpersWhitelist) {
		externalHelpersWhitelist = options.externalHelpersWhitelist;
	}
