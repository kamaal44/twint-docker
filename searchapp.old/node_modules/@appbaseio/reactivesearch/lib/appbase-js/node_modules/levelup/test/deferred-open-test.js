...
buster.testCase('Deferred open()', {
  ...
  'put() and get() on pre-opened database': function putAndGetOnPreOpenedDatabase(done) {
    ...
    db.put.bind(db, 'k1', 'v1'), db.put.bind(db, 'k2', 'v2'), db.put.bind(db, 'k3', 'v3')], function () {
  ...
  'batch() on pre-opened database': function batchOnPreOpenedDatabase(done) {
    var location = common.nextLocation()
    // 1) open database without callback, opens in worker thread
    ,
        db = levelup(location, { createIfMissing: true, errorIfExists: true, encoding: 'utf8' });
    ...
    db.batch([{ type: 'put', key: 'k1', value: 'v1' }, { type: 'put', key: 'k2', value: 'v2' }, { type: 'put', key: 'k3', value: 'v3' }], function () {
      // 3) when the callbacks have returned, the database should be open and those values should be in
      //    verify that the values are there
      async.forEach([1, 2, 3], function (k, cb) {
        db.get('k' + k, function (err, v) {
  ...
  'chained batch() on pre-opened database': function chainedBatchOnPreOpenedDatabase(done) {
    var location = common.nextLocation()
    // 1) open database without callback, opens in worker thread
    ,
        db = levelup(location, { createIfMissing: true, errorIfExists: true, encoding: 'utf8' });
  ...
  'test deferred ReadStream': {
    ...
    'simple ReadStream': function simpleReadStream(done) {
  ...
  'maxListeners warning': function maxListenersWarning(done) {
    var location = common.nextLocation()
    // 1) open database without callback, opens in worker thread
    ,
        db = levelup(location, { createIfMissing: true, errorIfExists: true, encoding: 'utf8' }),
