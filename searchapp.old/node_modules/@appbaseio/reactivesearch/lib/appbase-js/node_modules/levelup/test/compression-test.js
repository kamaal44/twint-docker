...
var async = require('async'),
  ...
  return 'aaaaaaaaaa';
    ...
    verify = function verify(location, compression, done) {
  du(location, function (err, size) {
    ...
    if (compression) assert(size < dataSize, 'on-disk size (' + size + ') is less than data size (' + dataSize + ')');else assert(size >= dataSize, 'on-disk size (' + size + ') is greater than data size (' + dataSize + ')');
    done();
  });
}
...
,
    cycle = function cycle(db, compression, callback) {
...
buster.testCase('Compression', {
  ...
  'test data is compressed by default (db.put())': function testDataIsCompressedByDefaultDbPut(done) {
  ...
  'test data is not compressed with compression=false on open() (db.put())': function testDataIsNotCompressedWithCompressionFalseOnOpenDbPut(done) {
  ...
  'test data is compressed by default (db.batch())': function testDataIsCompressedByDefaultDbBatch(done) {
    this.openTestDatabase(function (db) {
