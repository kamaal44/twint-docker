...
var es2015 = function ES2015(ES, ops, expectedMissing) {
	...
	test('ToNumber', function (t) {
		...
		t.test('binary literals', function (st) {
			...
			st.equal(ES.ToNumber({ toString: function toString() {
					return '0b11';
				} }), 3, 'Object that toStrings to 0b11 is 3');

			st.equal(true, is(ES.ToNumber('0b12'), NaN), '0b12 is NaN');
			st.equal(true, is(ES.ToNumber({ toString: function toString() {
					return '0b112';
	...
	test('ToObject', function (t) {
		...
		forEach(v.numbers, function (number) {
			...
			t.equal(typeof obj === 'undefined' ? 'undefined' : _typeof(obj), 'object', 'number ' + number + ' coerces to object');
	...
	test('Call', function (t) {
		...
		var throwsIfNotCallable = function throwsIfNotCallable(notFunc) {
			...
			}, TypeError, debug(notFunc) + ' (' + (typeof notFunc === 'undefined' ? 'undefined' : _typeof(notFunc)) + ') is not callable');
		};
	...
	test('SpeciesConstructor', function (t) {
		...
		t.equal(ES.SpeciesConstructor({ constructor: undefined }, defaultConstructor), defaultConstructor, 'undefined constructor returns defaultConstructor');
	...
	var bothDescriptor = function bothDescriptor() {
		return { '[[Get]]': function Get() {}, '[[Value]]': true };
	};
	var accessorDescriptor = function accessorDescriptor() {
		return {
			'[[Get]]': function Get() {},
			'[[Enumerable]]': true,
			'[[Configurable]]': true
	...
	var mutatorDescriptor = function mutatorDescriptor() {
		return {
			'[[Set]]': function Set() {},
			'[[Enumerable]]': true,
			'[[Configurable]]': true
	...
	var genericDescriptor = function genericDescriptor() {
		return {
			'[[Configurable]]': true,
			'[[Enumerable]]': false
		};
	...
	test('HasOwnProperty', function (t) {
		...
		t.equal(ES.HasOwnProperty({ toString: 1 }, 'toString'), true, 'shadowed inherited own properties are own');
	...
	test('HasProperty', function (t) {
		...
		t.equal(ES.HasProperty({ toString: 1 }, 'toString'), true, 'object has shadowed inherited own properties');
	...
	test('RegExpExec', function (t) {
		...
		t.test('throws if a callable "exec" returns a non-null non-object', function (st) {
			...
			forEach(v.nonNullPrimitives, function (nonNullPrimitive) {
				st['throws'](function () {
					ES.RegExpExec({ exec: function exec() {
	...
	test('CreateDataProperty', function (t) {
		...
		forEach(v.propertyKeys, function (propertyKey) {
			...
			if (typeof Object.defineProperty === 'function') {
				...
				t.notEqual(nonWritable[propertyKey], sentinel, debug(sentinel) + ' is not installed on "' + debug(propertyKey) + '" on the object when key is nonwritable');
				...
				t.notEqual(nonConfigurable[propertyKey], sentinel, debug(sentinel) + ' is not installed on "' + debug(propertyKey) + '" on the object when key is nonconfigurable');
			}
		});
	...
	test('AdvanceStringIndex', function (t) {
		...
		var notInts = v.nonNumbers.concat(v.nonIntegerNumbers, [Infinity, -Infinity, NaN, [], new Date(), Math.pow(2, 53), -1]);
