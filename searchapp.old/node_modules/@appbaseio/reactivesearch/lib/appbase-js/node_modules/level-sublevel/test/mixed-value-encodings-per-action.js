...
test('subsections support mixed encodings per put', function (t) {
  ...
  foo.put('foo1', 'foo1-value', { valueEncoding: 'utf8' }, function () {
    bar.put('bar1', { obj: 'ect' }, { valueEncoding: 'json' }, function () {
      ...
      foo.createReadStream({ start: 'foo1', end: 'foo1\xff', valueEncoding: 'utf8' }).on('data', function (d) {
        foodata = d;
      ...
      bar.createReadStream({ start: 'bar1', end: 'bar1\xff', valueEncoding: 'json' }).on('data', function (d) {
        bardata = d;
      }).on('error', function (err) {
        barerr = err;
      }).on('end', function () {
