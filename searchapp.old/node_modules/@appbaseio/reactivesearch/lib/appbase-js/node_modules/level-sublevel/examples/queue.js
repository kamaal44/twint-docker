...
module.exports = function (input, jobs, map, work) {
  if (!work) work = map, map = function map(data) {
    return data.key;
  };
  ...
  if ('string' === typeof jobs) jobs = input.sublevel(jobs);
  ...
  function doJob(data) {
    ...
    if (!running[hash]) running[hash] = true;else return;
  ...
  input.pre(function (ch, add) {
    ...
    var hash = shasum(key);
    console.log('KEY', key);
    if (!pending[hash]) add({ key: Date.now(), value: key, type: 'put' }, jobs);else pending[hash] = (0 || pending[hash]) + 1;
  });
